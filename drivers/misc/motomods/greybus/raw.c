...
static struct class *pbsys_class;
static int raw_major;
static int pbsys_major;
...
static int gb_raw_connection_init(struct gb_connection *connection)
{
	...
	if (connection->protocol_id == GREYBUS_PROTOCOL_PACKETBUS) {
		raw->device->devt = MKDEV(pbsys_major, 0);
		raw->device->class = pbsys_class;
		raw->device->parent = &connection->bundle->dev;
		raw->device->release = gb_raw_dev_release;
		dev_set_name(raw->device, "gbsys");
	} else {
...
static struct gb_protocol packetbus_protocol = {
	.name			= "pb",
	.id			= GREYBUS_PROTOCOL_PACKETBUS,
	.major			= GB_RAW_VERSION_MAJOR,
	.minor			= GB_RAW_VERSION_MINOR,
	.connection_init	= gb_raw_connection_init,
	.connection_exit	= gb_raw_connection_exit,
...
static int raw_init(void)
{
	...
	pbsys_class = class_create(THIS_MODULE, "gb_sys");

	if (IS_ERR(raw_class)||IS_ERR(pbsys_class)) {
		retval = PTR_ERR(raw_class)|PTR_ERR(pbsys_class);
		goto error_chrdev;
	}

	retval = alloc_chrdev_region(&dev_raw, 0, NUM_MINORS, "gb_raw");
	if (retval < 0)
		goto error_chrdev;

	retval = alloc_chrdev_region(&dev_pbsys, 0, NUM_MINORS, "gb_sys");
	if (retval < 0) {
		unregister_chrdev_region(dev_raw, NUM_MINORS);
		goto error_chrdev;
	}

	raw_major = MAJOR(dev_raw);
	pbsys_major = MAJOR(dev_pbsys);

	retval = gb_protocol_register(&raw_protocol);
	if (retval)
		goto error_gb;

	retval = gb_protocol_register(&packetbus_protocol);
	if (retval) {
...
error_gb:
	unregister_chrdev_region(dev_raw, NUM_MINORS);
	unregister_chrdev_region(dev_pbsys, NUM_MINORS);
error_chrdev:
	class_destroy(raw_class);
	class_destroy(pbsys_class);
...
{
	gb_protocol_deregister(&raw_protocol);
	gb_protocol_deregister(&packetbus_protocol);
	unregister_chrdev_region(MKDEV(pbsys_major, 0), NUM_MINORS);
	class_destroy(pbsys_class);
